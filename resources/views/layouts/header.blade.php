<header class="z-40" :class="{ 'dark': $store.app.semidark && $store.app.menu === 'horizontal' }">
    {{-- متغير حالة الاشتراك للشركات (غير السوبر أدمن) --}}
    @auth
        @php
            $subscriptionBadge = null;
            if (Auth::user()->company_code !== 'SA') {
                $subscription = \App\Models\CompanySubscription::where(
                    'company_code',
                    Auth::user()->company_code,
                )->first();
                $subscriptionBadge = $subscription ? $subscription->detailed_status : null;
            }
        @endphp
    @endauth
        ...
        <div class="relative flex w-full items-center bg-white px-5 py-2.5 dark:bg-[#0e1726]">
            <div class="horizontal-logo flex items-center justify-between ltr:mr-2 rtl:ml-2 lg:hidden">
                <a href="{{ url('/') }}" class="main-logo flex shrink-0 items-center">
                    ...
                    <span
                        class="hidden align-middle text-2xl font-semibold transition-all duration-300 ltr:ml-1.5 rtl:mr-1.5 dark:text-white-light md:inline">{{ Auth::user()->BranchName->companyName->name }}</span> --}}
                </a>
            ...
            <div class="hidden ltr:mr-2 rtl:ml-2 sm:block">
                <ul class="flex items-center space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                    {{-- شارة حالة الاشتراك --}}
                    @auth
                        @if (isset($subscriptionBadge) && $subscriptionBadge)
                            @php
                                $badgeColors = [
                                    'active' => 'bg-success text-white',
                                    'expiring_soon' => 'bg-warning text-white animate-pulse',
                                    'grace_period' => 'bg-orange-500 text-white animate-pulse',
                                    'expired' => 'bg-danger text-white',
                                    'suspended' => 'bg-gray-500 text-white',
                                ];
                                $badgeClass = $badgeColors[$subscriptionBadge['status']] ?? 'bg-gray-400 text-white';
                                $badgeIcons = [
                                    'active' => '✓',
                                    'expiring_soon' => '⚠',
                                    'grace_period' => '⏰',
                                ...
                                $badgeIcon = $badgeIcons[$subscriptionBadge['status']] ?? '?';
                            @endphp
                            <li>
                                <div class="relative group">
                                    ...
                                    {{-- Tooltip للتفاصيل --}}
                                    @if (in_array($subscriptionBadge['status'], ['expiring_soon', 'grace_period', 'expired']))
                                        <div
                                            class="absolute top-full right-0 mt-2 w-48 p-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg border hidden group-hover:block z-50 text-xs text-gray-700 dark:text-gray-300">
            ...
            <div x-data="header"
                ...
                <div class="dropdown flex-shrink-0" x-data="dropdown" @click.outside="open = false">
                    <a href="javascript:;" class="group relative" @click="toggle()">
                        ...
                        <span><img class="inline w-8 ltr:-ml-1 rtl:-mr-1"
                    ...
                    <ul x-cloak="" x-show="open" x-transition="" x-transition.duration.300ms=""
                        ...
                        <li>
                            <a href="{{ route('password.change') }}" class="dark:hover:text-white" @click="toggle">
