...
@section('content')
    <div class="panel mt-6 max-w-3xl mx-auto">
        <div class="flex items-center justify-between gap-3 mb-5">
            <h3 class="text-lg font-semibold dark:text-white-light">🧯 إضافة تالف (مادة رئيسية)</h3>
            <a href="{{ url('warehouse/addMainMaterialsBranch') }}" class="btn btn-outline-secondary btn-sm">← رجوع</a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div class="p-4 rounded border dark:border-gray-700">
                ...
                <div class="text-xs text-gray-500 mt-1">الكود: <span class="font-mono">{{ $material->code }}</span></div>
            </div>
            <div class="p-4 rounded border dark:border-gray-700">
        ...
        <form method="POST" action="{{ route('warehouse.update', $material->code) }}" id="lossForm" target="_blank">
            @csrf
            @method('PUT')
            <input type="hidden" name="active" value="ReportInventoryLoss">
    ...
    <script>
        ...
        (function() {
            ...
            btn.addEventListener('click', function() {
                // اترك وقت لتنفيذ الطلب وفتح الفاتورة في تبويب جديد
                setTimeout(function() {
