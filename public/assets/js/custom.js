(function () {
    const $themeConfig = {
        ...
        rtlClass: 'rtl', // rtl, ltr (الافتراضي: من اليمين لليسار)
    ...
    window.addEventListener('load', function () {
        ...
        // تهيئة الثيم (فاتح/داكن/تلقائي) وتطبيق تفضيل النظام إن كان الثيم تلقائي
        if (typeof Alpine !== 'undefined' && Alpine.store && Alpine.store('app') && Alpine.store('app').initTheme) {
    ...
    document.addEventListener('alpine:init', () => {
        ...
        Alpine.store('app', {
            // theme: light | dark | system (يدعم الثيم الفاتح والداكن والتلقائي)
            theme: Alpine.$persist($themeConfig.theme),
            isDarkMode: Alpine.$persist(false),
            setDarkModeFromTheme() {
                const t = this.theme || 'light';
                if (t === 'light') this.isDarkMode = false;
                else if (t === 'dark') this.isDarkMode = true;
                else if (t === 'system') {
            ...
            toggleTheme(val) {
                ...
                this.theme = val;
                this.setDarkModeFromTheme();
            },
            initTheme() {
                this.setDarkModeFromTheme();
                if (this.theme === 'system' && window.matchMedia) {
                    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => this.setDarkModeFromTheme());
                }
            },
